...
/**
 * Bug checker to detect instantiation of {@link InputStreamReader} without specifying charset.
 */
@AutoService(BugChecker.class)
@BugPattern(
    name = "InputStreamReaderWithoutCharset",
    summary = "Charset must be specified when constructing a new java.io.InputStreamReader.",
    category = JDK,
    severity = ERROR)
public class InputStreamReaderWithoutCharset
    extends BugChecker
    implements NewClassTreeMatcher {

  private static final String INPUT_STREAM = InputStream.class.getName();
  private static final String INPUT_STREAM_READER = InputStreamReader.class.getName();

  private static final Matcher<ExpressionTree> constructor =
      Matchers.constructor().forClass(INPUT_STREAM_READER).withParameters(INPUT_STREAM);
